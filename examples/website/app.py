...
def ieprocess(image_file):
    ...
    #Compile to a file
    swnresults = ie.Compile("{:d}x{:d}x{:d}".format(224,224,3), "resnet18.onnx", 'save.bin')
...
def upload_file():
    ...
    # check if the post request has the file part
        ...
        if file and allowed_file(file.filename):
            ...
            file.save(filepath)
            print(filepath)
            rstring = ieprocess(filepath)
            print(rstring)
            return render_template('index.html', user_image = filepath, results=rstring)
